...
When Decrement == TRUE, there is no significant difference between the paternal investment of Revealers (Mean = `r round(mean(PI.dTrue.Revealers),0)` units) and Concealers (Mean = `r round(mean(PI.dTrue.Concealers),0)` units), with an effect size of `r round(cohen.d(PI.dTrue.Revealers, PI.dTrue.Concealers, paired = TRUE)$estimate,2)`.
