---
title: "The female rivalry hypothesis - Main Experiments (sweeping over random seed)"
...
```{r}
d <- 
  read.csv('data/ovulationRandomSeedMainExperiments.csv', header=T) %>%
  as_tibble() %>%
...
Our outcome variables are paternal investment (PI) and lifetime reproductive success (RS). There are 1200 rows in this dataset (3 experiments x 100 model runs x 2 decrement parameters x 2 female strategies). However, here, we sweep over random seed numbers to ensure they aren't affecting our main results.
...
When Decrement == FALSE, there is no difference between the reproductive success of Revealers (Mean = `r round(mean(RS.dFalse.Revealers),2)` children) and Concealers (Mean = `r round(mean(RS.dFalse.Concealers),2)` children), with a negligible effect size of 0.05.
...
When Decrement == TRUE, there is a significant difference between the reproductive success of Revealers (Mean = `r round(mean(RS.dTrue.Revealers),2)` children) and Concealers (Mean = `r round(mean(RS.dTrue.Concealers),2)` children), with a small effect size of -0.47.
...
When Decrement == FALSE, there is no significant difference between the paternal investment of Revealers (Mean = `r round(mean(PI.dFalse.Revealers),0)` units) and Concealers (Mean = `r round(mean(PI.dFalse.Concealers),0)` units), with a negligible effect size of 0.01.
...
When Decrement == TRUE, there is a significant difference between the paternal investment of Revealers (Mean = `r round(mean(PI.dTrue.Revealers),0)` units) and Concealers (Mean = `r round(mean(PI.dTrue.Concealers),0)` units), with a medium effect size of -0.56.
...
When Decrement == FALSE, there is no significant difference between the reproductive success of Revealers (Mean = `r round(mean(RS.dFalse.Revealers),2)` children) and Concealers (Mean = `r round(mean(RS.dFalse.Concealers),2)` children), with a negligible effect size of -0.20.
...
When Decrement == TRUE, there is a significant difference between the reproductive success of Revealers (Mean = `r round(mean(RS.dTrue.Revealers),2)` children) and Concealers (Mean = `r round(mean(RS.dTrue.Concealers),2)` children), with a medium effect size of -0.51.
...
When Decrement == FALSE, there is no significant difference between the paternal investment of Revealers (Mean = `r round(mean(PI.dFalse.Revealers),0)` units) and Concealers (Mean = `r round(mean(PI.dFalse.Concealers),0)` units), with a negligible effect size of -0.21.
...
When Decrement == TRUE, there is a significant difference between the paternal investment of Revealers (Mean = `r round(mean(PI.dTrue.Revealers),0)` units) and Concealers (Mean = `r round(mean(PI.dTrue.Concealers),0)` units), with a medium effect size of -0.54.
...
When Decrement == FALSE, there is a significant difference between the reproductive success of Revealers (Mean = `r round(mean(RS.dFalse.Revealers),2)` children) and Concealers (Mean = `r round(mean(RS.dFalse.Concealers),2)` children), with a large effect size of -2.18.
...
When Decrement == TRUE, there is again a significant difference between the reproductive success of Revealers (Mean = `r round(mean(RS.dTrue.Revealers),2)` children) and Concealers (Mean = `r round(mean(RS.dTrue.Concealers),2)` children), with a large effect size of -2.32.
...
When Decrement == FALSE, there is a significant difference between the paternal investment of Revealers (Mean = `r round(mean(PI.dFalse.Revealers),0)` units) and Concealers (Mean = `r round(mean(PI.dFalse.Concealers),0)` units), with a large effect size of -2.52.
...
When Decrement == TRUE, there is a significant difference between the paternal investment of Revealers (Mean = `r round(mean(PI.dTrue.Revealers),0)` units) and Concealers (Mean = `r round(mean(PI.dTrue.Concealers),0)` units), with a large effect size of -2.66.
...
ggsave('figures/FigureS13.pdf', plot = p1, width = 180, height = 90, units = 'mm')
ggsave('figures/FigureS13.png', plot = p1, width = 180, height = 90, units = 'mm')
...
ggsave('figures/FigureS14.pdf', plot = p2, width = 180, height = 90, units = 'mm')
ggsave('figures/FigureS14.png', plot = p2, width = 180, height = 90, units = 'mm')
