...
```{r}
d <-
  read.csv('data/ovulationSensitivity_MalePromiscuity.csv') %>%
  as_tibble() %>%
...
There are 440 rows (11 parameters x 20 runs x 2 female strategies).
...
ggsave('figures/Figure2.pdf', width = 250, height = 100, units = 'mm')
ggsave('figures/Figure2.png', width = 250, height = 100, units = 'mm')
...
```{r}
d <-
  read.csv('data/ovulationSensitivity_AdDecay.csv') %>%
  as_tibble() %>%
  mutate(Run      = factor(Run),
         Sens     = factor(Sens),
         Strategy = factor(Strategy))
...
```{r}
d <-
  read.csv('data/ovulationSensitivity_costOcostS.csv') %>%
  as_tibble() %>%
  mutate(Run      = factor(Run),
         costO    = factor(costO),
         costS    = factor(costS),
...
This dataset has 1000 rows (5 costs-to-self x 5 costs-to-other x 20 runs x 2 female strategies).
...
Together, the cost of aggression to self AND other influence the difference between the strategies. To visualise this, we will load a pre-calculated dataset that contains the differences between strategies (in either reproductive success differences, or unit differences, depending on the outcome). We will now create two heatmaps that summarise how the cost of aggression affects the effect of strategy.

```{r}
d <-
  read.csv('data/ovulationSensitivity_costOcostSdiffs.csv') %>%
  as_tibble() %>%
  mutate(Run      = factor(Run),
         costO    = factor(costO),
         costS    = factor(costS),
...
```{r}
d <-
  read.csv('data/ovulationSensitivity_InitialFMod.csv') %>%
  as_tibble() %>%
  mutate(Run      = factor(Run),
         Sens     = factor(Sens),
         Strategy = factor(Strategy))
...
This dataset contains 360 rows (9 parameters x 20 runs x 2 female strategies).
...
```{r}
d <-
  read.csv('data/ovulationSensitivity_Search.csv') %>%
  as_tibble() %>%
  mutate(Run               = factor(Run),
